  ...
  useEffect(() => {
    const fetchOpenings = async () => {
      try {
        ...
        setMentorProjects(data.mentorProjects || []);
        setStudentOpenings(data.studentOpenings || []);
      } catch (err) {
        console.error("Failed to fetch project openings:", err);
      } finally {
        setLoading(false);
      }
    };
...
I came across your project on EduConnect and I'm interested in joining:
    ...
    <div className="p-8 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Mentor openings + student team opportunities (excluding your projects)
        </p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* LEFT: Mentor openings */}
        <Card className="border-2">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Briefcase className="w-5 h-5 text-primary" />
              Mentor Project Openings
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {mentorProjects.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                No mentor projects available right now.
              </p>
            ) : (
              mentorProjects.map(renderProjectCard)
            )}
          </CardContent>
        </Card>
        ...
        <Card className="border-2">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              Student Projects Looking for Teammates
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
